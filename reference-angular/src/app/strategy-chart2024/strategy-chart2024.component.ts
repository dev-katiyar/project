...
@Component({
  selector: 'app-strategy-chart2024',
  templateUrl: './strategy-chart2024.component.html',
  styleUrls: ['./strategy-chart2024.component.scss'],
})
export class StrategyChart2024Component implements OnInit {
  options = {};
  barClass = 'chart-big';
  chart: Chart;
  ...
  getSeriesTemplateAndYAxes(chartSettings) {
    ...
    this.chartInputs.flags_series.push(
      this.getBuyFlagSeriesTemplate(chartSettings['buy_flag_series']),
    );
    this.chartInputs.flags_series.push(
      this.getSellFlagSeriesTemplate(chartSettings['sell_flag_series']),
    );
  }
  ...
  getYAxisSettings(line, yAxis, pane) {
    return {
      title: {
        text: line['yAixsTitle'] ? line['yAixsTitle'] : line['name'],
        offset: 0,
        x: line['opposite'] ? 22 : -27.5,
      },
  ...
  fillSeriesWithData(chartInputs, chartData, chartSettings) {
    ...
    for (let rowObj of chartData) {
      ...
      if (buy_score == 1) {
        chartInputs.flags_series[0]['data'].push({ x: k, title: 'Buy' }); // Note Index 0
      } else if (buy_score == 2) {
        chartInputs.flags_series[0]['data'].push({ x: k, title: 'Strong Buy' }); // Note Index 0
      }

      let sell_score = rowObj[chartSettings.sell_flag_series.id]; // Fill Y-Axis 'Flags' Series - SELL

      if (sell_score == 1) {
        chartInputs.flags_series[1]['data'].push({ x: k, title: 'Sell' }); // Note Index 0
      } else if (sell_score == 2) {
        chartInputs.flags_series[1]['data'].push({ x: k, title: 'Strong Sell' }); // Note Index 0
      }
  ...
  setChartWithOptions(chartInputs) {
    this.options = {
      ...
      title: {
        text: this.chartSettings.chartTitle
          ? this.chartSettings.chartTitle
          : 'Strategy Builder',
      },
      subtitle: { text: '', x: 200 },
