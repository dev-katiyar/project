...
svmf_signature = {
    **defaults.svmf_defaults,
    'strategies': [
        {'key': 'ca', 'name': 'cross above', 'type': 'double', 'settings': [], 'options': [defaults.fixed_defaults, defaults.svmfsignal_defaults, ]},
        {'key': 'cb', 'name': 'cross below', 'type': 'double', 'settings': [], 'options': [defaults.fixed_defaults, defaults.svmfsignal_defaults, ]},
        {'key': 'gt', 'name': 'greater than', 'type': 'double', 'settings': [], 'options': [defaults.fixed_defaults, defaults.svmfsignal_defaults, ]},
        {'key': 'lt', 'name': 'less than', 'type': 'double', 'settings': [], 'options': [defaults.fixed_defaults, defaults.svmfsignal_defaults, ]},
    ]
}

class svMF(Indicator):
    ...
    def add_ind_cols(self, df, setting):
        try:
            ...
            if col_name not in df.columns.tolist():
                ...
                df[col_name + 'hist'] = df[col_name] - df[col_name + 'signal']
