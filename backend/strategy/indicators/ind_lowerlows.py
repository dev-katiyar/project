...
class LowerLows(Indicator):
    ...
    def add_ind_cols(self, df, setting):
        try:
            ...
            if col_name not in df.columns.tolist():
                minimas = self.get_lower_lows(df[target_col].values, order, count)
                df[col_name] = ''
                for min in minimas:
    ...
    def get_lower_lows(self, arr: np.array, order=5, K=2):
        # Get lows
        low_idx = argrelextrema(arr, np.less, order=order)[0]
        lows = arr[low_idx]
