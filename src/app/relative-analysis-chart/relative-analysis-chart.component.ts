...
@Component({
  selector: 'app-relative-analysis-chart',
  templateUrl: './relative-analysis-chart.component.html',
  styleUrls: ['./relative-analysis-chart.component.scss']
})
export class RelativeAnalysisChartComponent implements OnInit, OnChanges {
  ...
  plotBands = [
    ...
    // { color: '#adb5bd', from: -0.25, to: 0.25, label: {text: 'Neutral', style: {color: 'black', fontWeight: 'bold'}}, zIndex: 3 },
  ...
  prepareChartSeriesData(chartData) {
    ...
    let combinedKey =
      this.chartConfig.multiplier == -1
        ? this.chartConfig.yColKey2 + '_' + this.chartConfig.yColKey1
        : this.chartConfig.yColKey1 + '_' + this.chartConfig.yColKey2;
    ...
    let all_series = columns_series.concat(lines_series).concat(score_line_series);
  ...
  setChartOptions(chartSeries) {
    this.options = {
      ...
      yAxis: [
        /* Y-Axis [0] */
        {
          title: { text: 'RA Score', offset: 0, x: -35 },
          labels: { offset: 0, x: -5 },
          top: '0%',
