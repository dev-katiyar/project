...
import { FundamentalData } from '../models/symbol-data-types';
import { LiveService } from '../services/live.service';
import { TechnicalService } from '../services/technical.service';
import { MessageService } from 'primeng/api';

@Component({
  selector: 'app-table-chart-fundamentals',
  templateUrl: './table-chart-fundamentals.component.html',
  styleUrls: ['./table-chart-fundamentals.component.scss'],
})
export class TableChartFundamentalsComponent implements OnInit, OnChanges {
  ...
  selectedFundamentalDataType1 = this.fundamentalDataType1[0];
  ...
  constructor(private liveService: LiveService, private technicalService: TechnicalService, private messageService: MessageService) {}
  ...
  // onFundamentalDataTypeChange(event) {
  //   this.selectedFundamentalDataType1 = event.value;
  //   this.setDataForChart();
  // }
  ...
  setDataForChart() {
    ...
    const dataSerieConfig = JSON.parse(
      JSON.stringify(this.seriesConfigs[this.selectedFundamentalDataItem.code]),
    );
    dataSerieConfig['seriesData'].push(this.selectedChartDataFrequency.code);

    // Chart Configs for 1. and 2. above
    this.chartConfig.yAxes = [
      { title: { text: dataSerieConfig.seriesName }, opposite: false },
  ...
  addTrend() {
    if (!this.isTrendLineVisible) {
      ...
      const selectedSeriesConfig = JSON.parse(
        JSON.stringify(this.seriesConfigs[this.selectedFundamentalDataItem.code]),
      );
      selectedSeriesConfig['seriesData'].push(this.selectedChartDataFrequency.code);
      const seriesDataKeys = selectedSeriesConfig['seriesData'];
      const yCol = selectedSeriesConfig['yCol'];
      ...
      if (temp.length < 5) {
        this.messageService.add({ severity: "info", detail: "Not enough data for Trend Line", life: 1000 });
        return
      }
      seriesDataArr = temp;
