...
const PeerChart: React.FC<PeerChartProps> = ({ selected, peers, theme }) => {
  ...
  const all = [
    selected.data,
    ...peers.filter((p) => p.symbol !== selected.symbol),
  ]
...
const SectorPulsePage: React.FC = () => {
  ...
  const handleTickerClick = useCallback(
    async (ticker: TickerData, sector: string) => {
      setSelected({ symbol: ticker.symbol, sector, data: ticker });
      setPeers([]);
      setPeersLoading(true);

      try {
        const { data: peerSymbols } = await api.get<string[]>(
          `/peer/${ticker.symbol}`,
        );
  ...
  const neutral = allTickers.filter(
    (t) => Math.abs(t.priceChangePct ?? 0) < 0.01,
  ).length;
  const topSector = [...sectors]
    .filter((s) => s.tickers.length > 0)
    .sort(
      (a, b) =>
        (calcSectorAvg(b.tickers) ?? -Infinity) -
        (calcSectorAvg(a.tickers) ?? -Infinity),
    )[0];
  ...
  return (
    <div className="p-3" style={{ maxWidth: "1400px", margin: "0 auto" }}>
      ...
      {!pageLoading && (
        <div className="grid">
          {sectors.map((sector) => (
            <div key={sector.sector} className="col-12 md:col-6 lg:col-4">
              <SectorCard
                sector={sector}
        ...
        >
          ...
          >
            <div>
              ...
              >
                Click any bar to see that company's details. Compare{" "}
                {selected.symbol} vs its sector peers.
              </p>
            </div>
              ...
              <div style={{ textAlign: "center" }}>
                ...
                <div
                  style={{
                    ...
                    fontWeight: 800,
                    color:
                      (selected.data.priceChangePct ?? 0) >= 0
                        ? "var(--sv-gain)"
                        : "var(--sv-loss)",
                  }}
                >
          ...
          {/* Peer chart */}
          {peersLoading && <Skeleton height="200px" borderRadius="8px" />}
          {!peersLoading &&
            (peers.length > 0 || selected.data.priceChangePct != null) && (
              <>
                <div
                  style={{
                    fontSize: "0.8rem",
      ...
      >
        <span style={{ fontWeight: 700, color: "var(--sv-text-primary)" }}>
          <i
            className="pi pi-lightbulb mr-1"
            style={{ color: "var(--sv-accent)" }}
          />
          How to read this page:
        </span>{" "}
        Each colored tile represents a stock.{" "}
        <b style={{ color: HEAT_COLORS.strongGain.bg }}>Dark green</b> means the
        stock gained more than 2% today.{" "}
        <b style={{ color: HEAT_COLORS.strongLoss.bg }}>Dark red</b> means it
        fell more than 2%. Tiles are sorted best-to-worst within each sector.
        Click any tile to see how that stock compares to its peers.
      </div>
