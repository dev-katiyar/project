...
const AlertCard: React.FC<{ item: AlertItem; direction: "up" | "down" }> = ({
  item,
  direction,
}) => {
  ...
  const icon = isUp ? "pi-arrow-up" : "pi-arrow-down";
  const { prefix, body } = humaniseEvent(item.event);

  return (
    <div
      className="flex gap-2 p-2 border-round-lg"
      style={{
    ...
    >
      {/* Icon */}
      <div
        className="flex-shrink-0 flex align-items-center justify-content-center border-round"
        style={{
          width: 28,
          height: 28,
          background: bgColor,
      ...
      {/* Content */}
      <div className="flex-1 min-w-0 flex flex-column justify-content-between gap-1">
        {/* Top row: symbol + date */}
        <div className="flex align-items-center justify-content-between gap-1">
          ...
          <span
            className="sv-text-muted flex-shrink-0"
        ...
        <div
          className="text-xs"
          style={{ color: "var(--sv-text-primary)", lineHeight: 1.4 }}
        >
          {prefix && (
  ...
  <div className="flex flex-column align-items-center justify-content-center gap-3 sv-text-muted p-6 text-center">
    ...
    <div>
      ...
      <div className="text-xs sv-text-muted" style={{ maxWidth: 320 }}>
        Technical signals appear when stocks in your portfolio show meaningful
        price or momentum patterns.
      </div>
    </div>
  </div>
);
...
const AlertSkeletons: React.FC = () => (
  <div className="flex flex-column gap-2 p-3">
    {[1, 2, 3, 4, 5].map((i) => (
      <div
        key={i}
        className="flex gap-3 align-items-start p-3 border-round-xl"
        style={{
          background: "var(--sv-bg-surface)",
          border: "1px solid var(--sv-border)",
        }}
...
const TechSignalsTab: React.FC<Props> = ({ symbols, active }) => {
  ...
  const visibleAlerts: (AlertItem & { direction: "up" | "down" })[] = (() => {
    if (!alerts) return [];
    const up = (alerts.upsymbols ?? []).map((a) => ({
      ...a,
      direction: "up" as const,
    }));
    const down = (alerts.downsymbols ?? []).map((a) => ({
      ...a,
    ...
    return (
      <div className="flex flex-column align-items-center justify-content-center gap-3 sv-text-muted p-6 text-center">
        ...
        <div className="text-sm">
          Add holdings to your portfolio to see technical signals.
        </div>
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      {!loading && !error && (
        <div
          className="px-3 py-1 flex align-items-center gap-2 flex-wrap"
        ...
        >
          ...
          {total > 0 && (
            <div className="flex align-items-center gap-2 ml-auto">
              <span className="text-xs sv-text-muted sv-info-label">
                Sentiment
              </span>
              <div
              ...
              <span
                ...
                style={{
                  color:
                    upCount >= downCount ? "var(--sv-gain)" : "var(--sv-loss)",
                }}
              >
      ...
      {loading ? (
        ...
        <div className="flex flex-column align-items-center justify-content-center gap-3 p-6 text-center">
          <i
            className="pi pi-exclamation-triangle sv-text-loss"
            style={{ fontSize: "2rem" }}
          />
        ...
        <>
          ...
          {alerts && total > 0 && (
            <div
              className="mx-3 mt-2 border-round-lg px-3 py-2 flex align-items-start gap-2 text-xs sv-text-muted"
              style={{
                background: "var(--sv-accent-bg)",
                border: "1px solid var(--sv-border)",
            ...
            >
              <i
                className="pi pi-info-circle mt-1 flex-shrink-0"
                style={{ color: "var(--sv-accent)", fontSize: "0.8rem" }}
              />
              <span>
                These signals are for{" "}
                <strong>informational purposes only</strong> and are not buy or
                sell recommendations. Technical patterns help identify potential
                trends, but always consider your own financial situation before
